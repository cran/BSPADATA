hom_sar=function(y,X,W,nsim,burn,step,b_pri,B_pri,r_pri,lambda_pri,beta_0,sigma2_0,rho_0,kernel=NULL,plot=TRUE)
{
  rowst=function(x){
  x1=c()
  x1=(x)/sum(x)}
...
Dev=-2*logV1_DIC
DIC=2*Dbar+Dev
list(Bestimado = Bestimado, Sigma2est = Sigma2est, Rhoest= Rhoest,DesvBeta=DesvBeta,DesvSigma2=DesvSigma2,DesvRho=DesvRho,AccRate=AccRate,BIC=BIC,DIC=DIC)
}
