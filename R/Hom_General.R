#####Modelo de regresi?n General Homoced?stico
hom_general=function(y,X,W1,W2=NULL,nsim,burn,step,b_pri,B_pri,r_pri,lambda_pri,beta_0,sigma2_0,rho_0,lambda_0,kernel=NULL,plot=TRUE,mateq=TRUE)
{
rowst=function(x){
  x1=c()
  x1=(x)/sum(x)}
...
dp <- (sigma2^(-nrow(X)/2))*det(A)*det(B)*exp(-0.5*fc.y/sigma2)*exp(-0.5*fc.beta)*fc.sigma2
dp
}
...
T.val1=min(1,(p1.1*q1.1)/(p2.1*q2.1))
u<-runif(1)
if (u <=T.val1) {
Rho <- rho.now
ind[i,1] = 1
...
q1.1=dproposallambda(lambda.now)
q2.2=dproposallambda(Lambda)
T.val2=min(1,(p1.1*q1.1)/(p2.1*q2.2))
u<-runif(1)
if (u <=T.val2) {
Lambda <- lambda.now
ind[i,2] = 1
...
if(plot=="TRUE"){
 for (i in 1:ncol(X)) {
            dev.new()
            plot(beta.mcmc[, i], main = paste("Complete chain for beta",
                i), xlab = "number of iterations", ylab = paste("parameter beta",
                i),type="l",cex.main=1.5,cex.lab=1.5,cex.axis=1.5)}
dev.new()
plot(sigma2.mcmc,main ="Complete chain for Sigma^2", xlab = "number of iterations", ylab ="parameter sigma^2",type="l",cex.main=1.5,cex.lab=1.5,cex.axis=1.5)
dev.new()
plot(rho.mcmc,main ="Complete chain for Rho", xlab = "number of iterations", ylab ="parameter rho",type="l",cex.main=1.5,cex.lab=1.5,cex.axis=1.5)
dev.new()
plot(lambda.mcmc,main ="Complete chain for Lambda", xlab = "number of iterations", ylab ="parameter lambda",type="l",cex.main=1.5,cex.lab=1.5,cex.axis=1.5)
}
...
logV1_DIC=(-(nrow(X)/2)*log(pi))+log(detB)+log(detA)-0.5*log(detS)-0.5*t(Yg)%*%solve(Sigma)%*%Yg
Dev=-2*logV1_DIC
DIC=2*Dbar+Dev
yestimado=solve(diag(nrow(X))-Rhoest*matstand)%*%X%*%Bestimado
residuals=(y)-yestimado
list(Bestimado=Bestimado,Sigma2est=Sigma2est,Rhoest=Rhoest,Lambdaest=Lambdaest,AccRateRho=AccRate1,AccRateLambda=AccRate2,DesvBeta=DesvBeta,DesvSigma2=DesvSigma2,DesvRho=DesvRho,DesvLambda=DesvLambda,BIC=BIC,DIC=DIC)
}
